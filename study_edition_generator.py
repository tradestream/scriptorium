...
- Integration with EsotericAnalyzer for automated esoteric commentary layer
- Integration with LiteraryAnalyzer for automated literary commentary layer
...
class StudyEdition:
    ...
    # ------------------------------------------------------------------
    # AUTOMATIC COMMENTARY FROM LITERARY ANALYZER
    # ------------------------------------------------------------------

    def generate_literary_commentary(self, analyzer_results: dict,
                                      mode: str = 'poetry') -> Chapter:
        """
        Generate a commentary chapter from LiteraryAnalyzer results.
        Works for both poetry and prose mode results.
        Returns the generated Chapter for further customization.
        """
        mode_label = "poetry" if mode == 'poetry' else 'prose'
        ch = Chapter(
            title="Literary Analysis Commentary",
            subtitle=f"Computational close reading ({mode_label} mode) — 35-method literary analyzer",
            introduction=(
                "The following commentary has been generated by computational analysis "
                "using methods derived from: Pinsky's *The Sounds of Poetry*, Hollander's "
                "*Rhyme's Reason*, Ciardi's *How Does a Poem Mean*, and Foster's *How to "
                "Read Literature/Poetry Like a Professor*. "
                "These findings are starting points for interpretation, not conclusions."
            ),
        )

        score = analyzer_results.get('composite_literary_score', 0)
        interp = analyzer_results.get('score_interpretation', '')
        ch.add_section_header("Composite Score", level=1)
        ch.add_paragraph(
            f"Composite Literary Score: {score} — {interp}",
            css_class="indent",
        )
        ...
        method_groups = {
            'Prosody & Sound': [
                'meter', 'rhyme_scheme', 'alliteration', 'assonance',
                'consonance', 'enjambment', 'caesura', 'sound_meaning',
            ],
            'Form & Structure': [
                'fixed_form', 'stanza_structure', 'volta', 'refrain',
                'line_length', 'closure',
            ],
            'Figurative Language': [
                'metaphor_simile', 'image_clustering', 'concrete_abstract',
                'symbol_markers',
            ],
            'Diction & Vocabulary': [
                'etymology_register', 'vocabulary_richness', 'register_mixing',
                'pos_profile',
            ],
            'Narrative & Intertextual': [
                'archetype', 'allusion', 'seasonal_weather', 'intertextual',
            ],
            'Speaker & Performance': [
                'dramatic_monologue', 'apostrophe', 'tone', 'persona',
            ],
            'Cross-Domain (Shared)': [
                'contradiction', 'irony', 'silence', 'polysemy', 'symmetry',
            ],
        }
        ...
        for group_name, method_keys in method_groups.items():
            ...
            for key, data in group_methods:
                ...
                # Add specific findings based on method type
                self._add_literary_findings(ch, key, data)

        self.chapters.append(ch)
        return ch

    def _add_literary_findings(self, ch: Chapter, key: str, data: dict):
        """Add method-specific findings from the literary analyzer to a chapter."""

        # --- Prosody & Sound ---
        if key == 'meter':
            dom = data.get('dominant_foot', '')
            if dom:
                ch.add_paragraph(
                    f"Dominant metrical foot: {dom} "
                    f"(regularity: {data.get('regularity', 0):.2f})",
                    css_class="indent",
                )
            patterns = data.get('stress_patterns', [])
            if patterns:
                shown = patterns[:4]
                ch.add_paragraph(
                    f"Sample stress patterns: {', '.join(shown)}",
                    css_class="indent",
                )

        elif key == 'rhyme_scheme':
            scheme = data.get('scheme_string', '')
            if scheme:
                ch.add_paragraph(f"Rhyme scheme: {scheme}", css_class="indent")
            density = data.get('rhyme_density', 0)
            pairs = data.get('rhyme_pairs', 0)
            form = data.get('detected_form', '')
            details = []
            if pairs:
                details.append(f"{pairs} rhyme pairs")
            if density:
                details.append(f"{density:.0%} density")
            if form:
                details.append(f"form: {form}")
            if details:
                ch.add_paragraph(
                    f"Rhyme analysis: {', '.join(details)}",
                    css_class="indent",
                )
            internal = data.get('internal_rhymes', [])
            if internal:
                for ir in internal[:4]:
                    words = ir.get('words', ())
                    if words:
                        ch.add_paragraph(
                            f"Internal rhyme (line {ir.get('line', '?')}): "
                            f"{'/'.join(str(w) for w in words)}",
                            css_class="indent",
                        )

        elif key == 'alliteration':
            clusters = data.get('clusters', [])
            if clusters:
                for c in clusters[:5]:
                    ch.add_paragraph(
                        f"Alliterative cluster ({c.get('sound', '?')}): "
                        f"\"{c.get('text', '')[:120]}\"",
                        css_class="indent",
                    )

        elif key == 'assonance':
            patterns = data.get('vowel_patterns', [])
            if patterns:
                for p in patterns[:4]:
                    ch.add_paragraph(
                        f"Assonance ({p.get('vowel', '?')}): "
                        f"\"{p.get('text', '')[:120]}\"",
                        css_class="indent",
                    )

        elif key == 'consonance':
            patterns = data.get('consonant_patterns', [])
            if patterns:
                for p in patterns[:4]:
                    ch.add_paragraph(
                        f"Consonance ({p.get('consonant', '?')}): "
                        f"\"{p.get('text', '')[:120]}\"",
                        css_class="indent",
                    )
            ...
            if instances:
                for e in instances[:4]:
                    ch.add_paragraph(
                        f"Enjambment (lines {e.get('line', '?')}–{e.get('line', 0)+1}): "
                        f"\"{e.get('text', '')[:120]}\"",
                        css_class="indent",
                    )

        elif key == 'caesura':
            count = data.get('caesura_count', 0)
            if count:
                ch.add_paragraph(
                    f"Caesurae detected: {count} "
                    f"(density: {data.get('density', 0):.2f})",
                    css_class="indent",
                )

        elif key == 'sound_meaning':
            correspondences = data.get('correspondences', [])
            if correspondences:
                for c in correspondences[:4]:
                    ch.add_paragraph(
                        f"Sound–meaning: {c.get('category', '?')} sounds "
                        f"({c.get('description', '')})",
                        css_class="indent",
                    )
        ...
        # --- Form & Structure ---
            ...
            form = data.get('detected_form', '')
            if form:
                ch.add_paragraph(f"Fixed form detected: {form}", css_class="indent")
            candidates = data.get('form_candidates', [])
            if candidates:
                ch.add_paragraph(
                    f"Possible forms: {', '.join(candidates[:5])}",
                    css_class="indent",
                )

        elif key == 'stanza_structure':
            pattern = data.get('stanza_pattern', '')
            if pattern:
                ch.add_paragraph(
                    f"Stanza structure: {pattern}",
                    css_class="indent",
                )

        elif key == 'volta':
            location = data.get('volta_location', '')
            if location:
                ch.add_paragraph(
                    f"Turn/volta detected at: {location}",
                    css_class="indent",
                )
            markers = data.get('turn_markers', [])
            if markers:
                ch.add_paragraph(
                    f"Turn markers: {', '.join(str(m) for m in markers[:6])}",
                    css_class="indent",
                )

        elif key == 'refrain':
            refrains = data.get('refrains', [])
            if refrains:
                for r in refrains[:3]:
                    ch.add_paragraph(
                        f"Refrain ({r.get('count', 0)}x): \"{r.get('text', '')[:100]}\"",
                        css_class="indent",
                    )

        elif key == 'line_length':
            avg = data.get('avg_syllables', 0)
            variance = data.get('variance', 0)
            if avg:
                ch.add_paragraph(
                    f"Average line length: {avg:.1f} syllables "
            ...
            ch.add_paragraph(
                f"Closure strength: {data.get('score', 0):.2f} — "
                f"{data.get('interpretation', 'No closure analysis available')}",
                css_class="indent",
            )

        # --- Figurative Language ---
        elif key == 'metaphor_simile':
            similes = data.get('similes', [])
        ...
        elif key == 'image_clustering':
            clusters = data.get('clusters', {})
            if clusters:
                for domain, images in list(clusters.items())[:4]:
                    ch.add_paragraph(
                        f"Image cluster [{domain}]: {', '.join(images[:6])}",
                        css_class="indent",
                    )

        elif key == 'concrete_abstract':
            ratio = data.get('concrete_ratio', 0)
            ch.add_paragraph(
                f"Concrete/abstract ratio: {ratio:.2f} "
                f"(1.0 = fully concrete, 0.0 = fully abstract)",
                css_class="indent",
            )

        elif key == 'symbol_markers':
            symbols = data.get('symbols', [])
            if symbols:
                for s in symbols[:5]:
                    ch.add_paragraph(
                        f"Symbolic marker: {s.get('word', '?')} "
                        f"({s.get('category', 'uncategorized')})",
                        css_class="indent",
                    )

        # --- Diction & Vocabulary ---
        elif key == 'etymology_register':
            ratio = data.get('anglo_saxon_ratio', 0)
            ch.add_paragraph(
                f"Anglo-Saxon to Latinate ratio: {ratio:.2f}",
                css_class="indent",
            )

        elif key == 'vocabulary_richness':
            ttr = data.get('type_token_ratio', 0)
            hapax = data.get('hapax_ratio', 0)
            ch.add_paragraph(
                f"Type-token ratio: {ttr:.3f}, Hapax ratio: {hapax:.3f}",
                css_class="indent",
            )

        elif key == 'register_mixing':
            examples = data.get('examples', [])
            if examples:
                for e in examples[:3]:
                    ch.add_paragraph(
                        f"Register shift: \"{e[:120]}\"",
                        css_class="indent",
                    )

        elif key == 'pos_profile':
            profile = data.get('profile', {})
            if profile:
                top = sorted(profile.items(), key=lambda x: -x[1])[:5]
                parts_str = ", ".join(f"{k}: {v:.1%}" for k, v in top)
                ch.add_paragraph(
                    f"POS distribution: {parts_str}",
                    css_class="indent",
                )
        ...
        # --- Narrative & Intertextual ---
            ...
            if patterns:
                ch.add_paragraph(
                    f"Archetypal patterns: {', '.join(patterns[:5])}",
                    css_class="indent",
                )

        elif key == 'allusion':
            found = data.get('allusions', [])
            if found:
                for a in found[:5]:
                    ch.add_paragraph(
                        f"Allusion ({a.get('type', '?')}): {a.get('text', '')[:120]}",
                        css_class="indent",
                    )

        elif key == 'seasonal_weather':
            seasons = data.get('seasons', [])
            weather = data.get('weather', [])
            if seasons:
                ch.add_paragraph(
                    f"Seasonal markers: {', '.join(seasons[:5])}",
                    css_class="indent",
                )
            if weather:
                ch.add_paragraph(
                    f"Weather vocabulary: {', '.join(weather[:5])}",
                    css_class="indent",
                )

        elif key == 'intertextual':
            refs = data.get('references', [])
            if refs:
                for r in refs[:4]:
                    ch.add_paragraph(
                        f"Intertextual reference: \"{r[:120]}\"",
                        css_class="indent",
                    )

        # --- Speaker & Performance ---
        elif key == 'dramatic_monologue':
            indicators = data.get('indicators', [])
            if indicators:
                ch.add_paragraph(
                    f"Dramatic monologue indicators: {', '.join(indicators[:5])}",
                    css_class="indent",
                )
            ...
            shifts = data.get('tone_shifts', [])
            if shifts:
                for s in shifts[:3]:
                    ch.add_paragraph(
                        f"Tone shift: {s.get('from', '?')} → {s.get('to', '?')} "
                        f"(at {s.get('location', '?')})",
                        css_class="indent",
                    )

        elif key == 'persona':
            markers = data.get('persona_markers', [])
            if markers:
                ch.add_paragraph(
                    f"Persona/speaker markers: {', '.join(markers[:5])}",
                    css_class="indent",
                )

        # --- Shared Methods ---
        elif key == 'contradiction' and data.get('contradictions'):
            for c in data['contradictions'][:5]:
                ch.add_paragraph(
                    f"Contradiction: \"{c.get('sentence_a', '')[:100]}\" vs. "
                    f"\"{c.get('sentence_b', '')[:100]}\"",
                    css_class="indent",
                )

        elif key == 'irony':
            instances = data.get('irony_instances', [])
            if instances:
                for i in instances[:3]:
                    ch.add_paragraph(
                        f"Irony marker: \"{i[:120]}\"",
                        css_class="indent",
                    )
            ...
            if gaps:
                for g in gaps[:3]:
                    ch.add_paragraph(
                        f"Silence/omission: {g[:120]}",
                        css_class="indent",
                    )

        elif key == 'polysemy':
            words = data.get('polysemous_words', [])
            if words:
                ch.add_paragraph(
                    f"Polysemous words: {', '.join(words[:8])}",
                    css_class="indent",
                )

        elif key == 'symmetry':
            sym_type = data.get('symmetry_type', '')
            if sym_type:
                ch.add_paragraph(
                    f"Structural symmetry: {sym_type} "
                    f"(strength: {data.get('score', 0):.2f})",
                    css_class="indent",
                )
