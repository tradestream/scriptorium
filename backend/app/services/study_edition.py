...
def build_study_edition_from_analysis(
    edition_id: int,
    markdown_text: str,
    computational_results: dict,
    llm_analyses: list[dict] = None,
    kobo_color: bool = True,
) -> bytes:
    """Build a complete study edition EPUB from Scriptorium analysis results.

    Integrates:
    - Original text (from cached markdown)
    - 35-tool computational analysis as footnotes and commentary
    - LLM analysis as thematic essays
    - Esoteric heatmap as section-level annotations

    Returns EPUB bytes (or KEPUB bytes if kobo_color=True).
    """
    import sqlite3
    import tempfile
    ...
    try:
        row = conn.execute("""
            SELECT w.title, w.subtitle, w.description,
    ...
    edition = StudyEdition(
        title=title,
        author=authors,
        subtitle=f"Study Edition with Esoteric Commentary{' — ' + subtitle if subtitle else ''}",
        editor="Scriptorium",
    )

    # Set general introduction
    v2 = computational_results.get("engine_v2", {})
    score = v2.get("overall_score", "N/A")
    finding_count = v2.get("finding_count", 0)
    edition.set_general_introduction(
        f"This study edition of *{title}* by {authors} integrates the results of "
        f"computational esoteric analysis (35 pattern-detection tools, overall score: {score}, "
        f"{finding_count} findings) with LLM-powered interpretive commentary.\n\n"
        f"The computational tools are based on the methods of Leo Strauss, Arthur Melzer, "
        f"Seth Benardete, Maimonides, Al-Farabi, and the broader tradition of esoteric "
        f"textual interpretation.\n\n"
        f"**How to read this edition:**\n\n"
        f"- Letter-keyed footnotes (a, b, c...) provide esoteric commentary on specific passages\n"
        f"- Cross-references [in brackets] link to related passages\n"
        f"- Section headers mark structurally significant divisions\n"
        f"- Thematic essays at the end provide extended LLM analysis\n"
        f"- The Esoteric Analysis Commentary chapter presents all computational findings"
    )

    # Import the original text as chapters
    from app.services.markdown import strip_yaml_frontmatter
    clean_text = strip_yaml_frontmatter(markdown_text)
    edition.import_text(clean_text)

    # Add esoteric heatmap annotations as footnotes to relevant sections
    heatmap = v2.get("structural_summary", {}).get("heatmap", [])
    hot_sections = [h for h in heatmap if h.get("heat_level") in ("HIGH", "MEDIUM")]

    if hot_sections and edition.chapters:
        # Add a note to the first chapter about hot sections
        if edition.chapters[0].elements:
            hot_list = ", ".join(
                f"{h.get('section', '?')[:30]} ({h.get('heat_level')})"
                for h in sorted(hot_sections, key=lambda x: -x.get("intensity", 0))[:5]
            )
            edition.chapters[0].add_sidebar_essay(
                "Esoteric Hotspots",
                f"The computational analysis identified these sections as having the highest "
                f"concentration of esoteric signals: {hot_list}. These sections deserve "
                f"especially careful reading."
            )

    # Generate computational commentary chapter
    edition.generate_esoteric_commentary(computational_results)

    # Add LLM analyses as thematic essays
    if llm_analyses:
        for analysis in llm_analyses:
            atitle = analysis.get("title", "Analysis")
            ...
            if content:
                ...
                if model:
                    essay_content += f"\n\n*Generated by {model}*"
                edition.add_essay(atitle, essay_content, author="Scriptorium AI")

    # Export
    with tempfile.NamedTemporaryFile(suffix=".kepub.epub" if kobo_color else ".epub",
                                      delete=False) as tmp:
        if kobo_color:
            edition.export_kepub(tmp.name)
        else:
            edition.export_epub(tmp.name)
        tmp.seek(0)
