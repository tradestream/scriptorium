<script lang="ts">
  ...
  function cancelEdit() { editingId = null; editingName = ''; renameError = ''; }
  ...
  function cancelMerge() { mergeSourceId = null; mergeError = ''; }
  ...
  const currentList = $derived(
    ...
    activeTab === 'tags' ? filteredTags : filteredSeries
  );
...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
  <div class="mb-6">
    ...
    <p class="mt-1 text-muted-foreground">Rename, merge, and remove Authors, Tags, and Series</p>
  </div>

  <!-- Tabs -->
  <div class="mb-5 flex gap-1 rounded-lg border bg-muted/40 p-1 w-fit">
    {#each (['authors', 'tags', 'series'] as const) as tab}
      <button
  ...
  <!-- Merge mode banner -->
  {#if mergeSourceId}
    {@const sourceName = currentList.find(i => i.id === mergeSourceId)?.name}
    <div class="mb-4 flex items-center justify-between rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
      <p class="text-sm">
        ...
        — click another entry to absorb it (all books reassigned to target)
      </p>
  ...
  {:else}
    <div class="divide-y rounded-lg border">
      {#each currentList as item (item.id)}
        ...
        {@const isMergeSource = mergeSourceId === item.id}
        ...
        <div
          ...
          onclick={() => { if (isTarget) mergeInto(item.id); }}
        >
          {#if isEditing}
            <input
          ...
          {:else}
            <span class="flex-1 text-sm {isMergeSource ? 'font-semibold text-amber-700 dark:text-amber-400' : 'font-medium'}">{item.name}</span>
            <Badge variant="secondary" class="shrink-0 tabular-nums text-xs">
              <BookOpen class="mr-1 h-2.5 w-2.5" />{item.book_count}
            </Badge>
            {#if !mergeSourceId}
              <div class="invisible flex shrink-0 items-center gap-1.5 group-hover:visible">
                <button onclick={(e) => { e.stopPropagation(); startEdit(item.id, item.name); }} title="Rename" class="text-muted-foreground/50 hover:text-foreground transition-colors">
                  <Pencil class="h-3.5 w-3.5" />
                </button>
                <button onclick={(e) => { e.stopPropagation(); startMerge(item.id); }} title="Merge into another" class="text-muted-foreground/50 hover:text-foreground transition-colors">
                  <GitMerge class="h-3.5 w-3.5" />
                </button>
                {#if item.book_count === 0}
                  <button onclick={(e) => { e.stopPropagation(); deleteEntity(item.id); }} title="Delete (no books)" class="text-muted-foreground/50 hover:text-destructive transition-colors">
